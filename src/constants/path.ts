export const PATH = {
  HOME: '/',
  SCHEDULE: '/schedule',
  SCHEDULE_DETAIL: 'detail/:id',
  SCHEDULE_ADD: 'add',
  SCHEDULE_EDIT: 'edit',
  SALARY: '/salary',
  SALARY_DETAIL: 'detail/:year/:month',
  SALARY_CORRECTION_HISTORY: 'CorrectionHistory',
  SALARY_CORRECTION_DETAIL: 'CorrectionHistory/CorrectionDetail/:id',
  SALARY_CORRECTION_EDIT: 'CorrectionHistory/CorrectionEdit/:id',
  PROFILE: '/profile',
  PROFILE_EDIT: 'edit',
  SIGNIN: '/signin',
};

export const PATH_TITLE = {
  HOME: '홈',
  SCHEDULE: '일정',
  SALARY: '급여',
  SALARY_CORRECTION_HISTORY: '정정 내역',
  SALARY_CORRECTION_DETAIL: '정정 상세',
  SALARY_CORRECTION_EDIT: '정정 수정',
  PROFILE: '프로필',
};
